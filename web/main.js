...
function render_content(obj) {
	...
	if(window.location.href.split('/')[5] == undefined) {
		...
		x = window.location.href.split('/')[5];
	}
...
//Dynamic link function for making dynamic links
function d_links() {
...
//Initialising the dynamic links in current page
d_links();

//Event handler for change in the url of the page
window.onpopstate=()=>{
	load_page();
};
